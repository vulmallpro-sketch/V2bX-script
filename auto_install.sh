...
install_v2bx() {
    ...
    # 通过管道传入 "n" 跳过 install.sh 末尾的交互式配置向导提示
    # （本脚本会在 install.sh 执行完之后自行生成配置文件）
    if [[ -n "$v2bx_version" ]]; then
        echo "n" | bash <(curl -Ls https://raw.githubusercontent.com/wyx2685/V2bX-script/master/install.sh) "$v2bx_version"
    else
        echo "n" | bash <(curl -Ls https://raw.githubusercontent.com/wyx2685/V2bX-script/master/install.sh)
    fi
    if [[ $? -ne 0 ]]; then
